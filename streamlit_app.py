...
st.set_page_config(page_title="Berry Phase Holonomy", layout="wide")
...
st.markdown('<h1 style="text-align: center;">🌀 Berry Phase Holonomy</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Geometric phase accumulation | Path‑dependent structure | Non‑Markovian memory detection</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Berry Phase")
...
st.sidebar.markdown(f"**Steps:** {config.N_STEPS} | **All macros used**")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'berry_phase_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Berry phase: {etf['berry_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Berry Phase"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Berry Phase", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("🌀 Top ETFs by Berry Phase Holonomy (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Berry phase** is a geometric phase accumulated by a system driven cyclically.
        - Nonzero holonomy signals path‑dependent, non‑integrable structure.
        - We track the state vector (mean, volatility, skewness, kurtosis) driven by macro regimes.
        - The Berry phase magnitude measures **non‑Markovian memory** beyond standard models.
        - Higher score → stronger path‑dependent structure → potential alpha.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the Berry phase scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Berry Phase Holonomy | Geometric phase for non‑Markovian structure")
