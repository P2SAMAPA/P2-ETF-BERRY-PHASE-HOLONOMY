...
from berry_phase import berry_phase_score
...
def run_for_window(returns, macro_df, window_days):
    ...
    for ticker in ret_window.columns:
        ...
        # Add small ticker variation to break ties (0-0.0001 range)
    ...
    top_etfs = [{"ticker": t, "berry_score_norm": s, "raw_score": raw_scores[t]} for t, s in sorted_norm[:config.TOP_N]]
    return {
        "window": window_days,
        "top_etfs": top_etfs,
...
def main():
    ...
    out_file = f"output/berry_phase_{timestamp}.json"
